...
CHART_DIR ?= $(CURDIR)/traefikee
TMPDIR ?= /tmp
HELM_REPO ?= $(CURDIR)/repo
LINT_USE_DOCKER ?= true
LINT_CMD ?= ct lint --config=lint/ct.yaml
PROJECT ?= traefik/traefikee-helm-chart
VERSION ?= $(shell cat traefikee/Chart.yaml | grep 'version: ' | awk '{ print $$2 }')
...
unit-test: helm-unittest
	...
	@helm unittest --helm3 --color --update-snapshot ./traefikee
...
package: global-requirements $(DIST_DIR) $(HELM_REPO) build full-yaml
	...
	@helm repo index --merge $(HELM_REPO)/index.yaml --url https://helm.traefik.io/traefikee/ $(HELM_REPO)
...
full-yaml:
	...
	@helm template traefikee traefikee > $(DIST_DIR)/yaml/$(VERSION).yaml

install: global-requirements $(DIST_DIR)
	@helm install traefikee $(CHART_DIR)

uninstall: global-requirements $(DIST_DIR)
	@helm uninstall traefikee $(CHART_DIR)
	clean
